...
from .priorities import ROTATION_ORDER, get_topic_priority
...
def spaced_recall_offsets() -> List[int]:
    """Return spaced recall pattern offsets (days after learning)."""
    return [1, 3, 5, 7, 9]


def build_rotation_series(n_days: int) -> List[str]:
    """Rotate topics cyclically for n_days."""
    return [ROTATION_ORDER[i % len(ROTATION_ORDER)] for i in range(n_days)]
...
def build_schedule(
    ...
    Returns a dict with:
      - meta: plan metadata
      - schedule: list of daily plans (using `hours` fields throughout)
    ...
    for w_idx, w_start in enumerate(range(0, total_days, 7), start=1):
        ...
        mock_count = sum(1 for d in block if d["is_mock_day"])

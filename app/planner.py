...
def build_weighted_rotation(
    ...
    P1 topics appear 3x more often than P3 topics.
    Custom weights and weakness data further adjust frequency.
    """
    ...
    for topic, data in SYLLABUS_TREE.items():
        ...
        # Weakness boost: topics with low accuracy get more time
    ...
    for offset in range(1, 4):
        if day_index - offset >= 0:
            ...
            if prev != theory_today:
                return prev
    all_topics = list(SYLLABUS_TREE.keys())
    for t in all_topics:
        if t != theory_today:
            return t
    return theory_today
...
def build_schedule(
    ...
    # Build weighted rotation considering custom weights and weakness
